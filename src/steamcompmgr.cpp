...
static void
create_color_mgmt_luts(const gamescope_color_mgmt_t &newColorMgmt,
  ...
  for (uint32_t nInputEOTF = 0; nInputEOTF < EOTF_Count; nInputEOTF++) {
    ...
    if (g_ColorMgmtLutsOverride[nInputEOTF].HasLuts()) {
      ...
      float flGain = 1.f;
      lut3d_t *pLook = g_ColorMgmtLooks[nInputEOTF].lutEdgeSize > 0
      ...
      if (inputEOTF == EOTF_Gamma22) {
        ...
        // The final display colorimetry is used to build the output mapping, as
        // we want a gamut-aware handling for sdrGamutWideness indepdendent of
        // the output encoding (for SDR data), and when mapping SDR -> PQ output
        // we only want to utilize a portion of the gamut the actual display can
        // reproduce
        buildSDRColorimetry(&inputColorimetry, &colorMapping,
        ...
        if (newColorMgmt.outputEncodingEOTF == EOTF_Gamma22) {
          ...
          // TODO: If hdrTonemapDisplayMetadata is invalid, use the one provided
          // by the display
          ...
          if (source.BIsValid() && dest.BIsValid()) {
            ...
            tonemapping.eetf2390.init(source,
          ...
          tonemapping.g22_luminance, flGain ); xwm_log.infof("source %f %f",
          source.flBlackPointNits, source.flWhitePointNits );
          ...
          // TODO: should we manipulate the output static metadata to reflect
          // the gain factor?
          tonemapping.g22_luminance = 1.f;
          // xwm_log.infof("PQ -> PQ");
        }

        buildPQColorimetry(&inputColorimetry, &colorMapping,
                           displayColorimetry);
      }

      calcColorTransform(&g_tmpLut1d, s_nLutSize1d, &g_tmpLut3d,
                         s_nLutEdgeSize3d, inputColorimetry, inputEOTF,
                         outputEncodingColorimetry,
                         newColorMgmt.outputEncodingEOTF, colorMapping,
                         newColorMgmt.nightmode, tonemapping, pLook, flGain);

      // Create quantized output luts
      for (size_t i = 0, end = g_tmpLut1d.dataR.size(); i < end; ++i) {
        outColorMgmtLuts[nInputEOTF].lut1d[4 * i + 0] =
            drm_quantize_lut_value(g_tmpLut1d.dataR[i]);
        outColorMgmtLuts[nInputEOTF].lut1d[4 * i + 1] =
            drm_quantize_lut_value(g_tmpLut1d.dataG[i]);
        outColorMgmtLuts[nInputEOTF].lut1d[4 * i + 2] =
            drm_quantize_lut_value(g_tmpLut1d.dataB[i]);
      ...
      for (size_t i = 0, end = g_tmpLut3d.data.size(); i < end; ++i) {
        outColorMgmtLuts[nInputEOTF].lut3d[4 * i + 0] =
            drm_quantize_lut_value(g_tmpLut3d.data[i].r);
        outColorMgmtLuts[nInputEOTF].lut3d[4 * i + 1] =
            drm_quantize_lut_value(g_tmpLut3d.data[i].g);
        outColorMgmtLuts[nInputEOTF].lut3d[4 * i + 2] =
            drm_quantize_lut_value(g_tmpLut3d.data[i].b);
    ...
    vulkan_update_luts(outColorMgmtLuts[nInputEOTF].vk_lut1d,
...
static void update_color_mgmt() {
  ...
  if (!BIsNested()) {
    drm_get_native_colorimetry(&g_DRM, &g_ColorMgmt.pending.displayColorimetry,
...
const uint32_t WS_OVERLAPPEDWINDOW = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU |
...
static std::unordered_map<struct wlr_buffer *, wlr_buffer_map_entry>
    wlr_buffer_map;
...
static void discard_ignore(xwayland_ctx_t *ctx, unsigned long sequence) {
...
static void set_ignore(xwayland_ctx_t *ctx, unsigned long sequence) {
...
static int should_ignore(xwayland_ctx_t *ctx, unsigned long sequence) {
  discard_ignore(ctx, sequence);
  return ctx->ignore_head && ctx->ignore_head->sequence == sequence;
}

static bool x_events_queued(xwayland_ctx_t *ctx) {
...
static steamcompmgr_win_t *find_win(xwayland_ctx_t *ctx, Window id,
                                    bool find_children = true) {
...
static steamcompmgr_win_t *find_win(xwayland_ctx_t *ctx,
...
static void destroy_buffer(struct wl_listener *listener, void *) {
...
static std::shared_ptr<commit_t> import_commit(
...
static int32_t window_last_done_commit_id(steamcompmgr_win_t *w) {
...
static void get_window_last_done_commit(steamcompmgr_win_t *w,
...
void calc_scale_factor_scaler(float &out_scale_x, float &out_scale_y,
...
void calc_scale_factor(float &out_scale_x, float &out_scale_y,
                       float sourceWidth, float sourceHeight) {
...
MouseCursor::MouseCursor(xwayland_ctx_t *ctx)
    : m_texture(0), m_dirty(true), m_imageEmpty(false), m_hideForMovement(true),
      m_ctx(ctx) {
...
void MouseCursor::checkSuspension() {
  ...
  if (window && window->ignoreNextClickForVisibility) {
    ...
    if (buttonMask &
      ...
      // Move the cursor back to where we left it if the window didn't want us
      // to give it hover/focus where we left it and we moved it before.
      if (window_wants_no_focus_when_mouse_hidden(window) && bWasHidden) {
        XWarpPointer(m_ctx->dpy, None, x11_win(m_ctx->focus.inputFocusWindow),
                     0, 0, 0, 0, m_lastX, m_lastY);
      }
    }
  }

  const bool suspended =
      get_time_in_milliseconds() - m_lastMovedTime > cursorHideTime;
  if (!m_hideForMovement && suspended) {
    ...
    if (window) {
      ...
      if (window_wants_no_focus_when_mouse_hidden(window)) {
        ...
        m_lastY = m_y;
        XWarpPointer(m_ctx->dpy, None, x11_win(m_ctx->focus.inputFocusWindow),
                     0, 0, 0, 0, window->xwayland().a.width - 1,
                     window->xwayland().a.height - 1);
      }
    }
...
void MouseCursor::warp(int x, int y) {
  XWarpPointer(m_ctx->dpy, None, x11_win(m_ctx->focus.inputFocusWindow), 0, 0,
               0, 0, x, y);
}

void MouseCursor::resetPosition() { warp(m_x, m_y); }
...
bool MouseCursor::setCursorImage(char *data, int w, int h, int hx, int hy) {
  ...
  if (!(ximage = XCreateImage(
            m_ctx->dpy, DefaultVisual(m_ctx->dpy, DefaultScreen(m_ctx->dpy)),
            32, ZPixmap, 0, data, w, h, 32, 0))) {
    xwm_log.errorf("Failed to make ximage for cursor");
    goto error_image;
  }

  if (!(pixmap = XCreatePixmap(m_ctx->dpy, DefaultRootWindow(m_ctx->dpy), w, h,
                               32))) {
  ...
  if (!(picture =
            XRenderCreatePicture(m_ctx->dpy, pixmap, pictformat, 0, NULL))) {
...
bool MouseCursor::setCursorImageByName(const char *name) {
  ...
  Cursor cursor =
      XCreateGlyphCursor(m_ctx->dpy, font, font, index, index + 1, &fg, &bg);
  if (!cursor)
    return false;
...
void MouseCursor::constrainPosition() {
  ...
  // Constrain it to the window; careful, the corners will leak due to a known X
  // server bug.
...
void MouseCursor::move(int x, int y) {
  ...
  if ((window != global_focus.inputFocusWindow ||
       !g_bPendingTouchMovement.exchange(false)) &&
      window->mouseMoved++ < 5)
    return;
...
bool MouseCursor::getTexture() {
  ...
  if (image->width && image->height) {
    cursorBuffer =
        std::make_shared<std::vector<uint32_t>>(surfaceWidth * surfaceHeight);
  ...
  m_texture = vulkan_create_texture_from_bits(
      surfaceWidth, surfaceHeight, image->width, image->height,
      DRM_FORMAT_ARGB8888, texCreateFlags, cursorBuffer->data());
  sdlwindow_cursor(std::move(cursorBuffer), image->width, image->height,
...
void MouseCursor::paint(steamcompmgr_win_t *window, steamcompmgr_win_t *fit,
  ...
  if (fit) {
    // If we have an override window, try to fit it in as long as it won't make
    // our scale go below 1.0.
    sourceWidth = std::max<uint32_t>(
        sourceWidth, clamp<int>(fit->xwayland().a.x + fit->xwayland().a.width,
                                0, currentOutputWidth));
    sourceHeight = std::max<uint32_t>(
        sourceHeight, clamp<int>(fit->xwayland().a.y + fit->xwayland().a.height,
                                 0, currentOutputHeight));
  }
  ...
  calc_scale_factor(currentScaleRatio_x, currentScaleRatio_y, sourceWidth,
                    sourceHeight);
  ...
  scaledX =
      (winX - window->xwayland().a.x) * currentScaleRatio_x + cursorOffsetX;
  scaledY =
      (winY - window->xwayland().a.y) * currentScaleRatio_y + cursorOffsetY;
...
void MouseCursor::updateCursorFeedback(bool bForce) {
  ...
  XChangeProperty(m_ctx->dpy, m_ctx->root,
                  m_ctx->atoms.gamescopeCursorVisibleFeedback, XA_CARDINAL, 32,
...
static void paint_cached_base_layer(const std::shared_ptr<commit_t> &commit,
...
static void paint_window(steamcompmgr_win_t *w, steamcompmgr_win_t *scaleW,
                         struct FrameInfo_t *frameInfo, MouseCursor *cursor,
  ...
  if (flags & PaintWindowFlag::BasePlane) {
    if (!lastCommit) {
      ...
      if (g_HeldCommits[HELD_COMMIT_BASE]) {
        paint_cached_base_layer(g_HeldCommits[HELD_COMMIT_BASE],
                                g_CachedPlanes[HELD_COMMIT_BASE], frameInfo,
                                flOpacityScale);
        return;
      }
    } else {
  ...
  if (notificationMode) {
    ...
    if (fit) {
      ...
      sourceHeight = std::max<uint32_t>(
          sourceHeight,
          clamp<int>(fit->xwayland().a.y + fit->xwayland().a.height, 0,
                     currentOutputHeight));
    }
  }
  ...
  if (sourceWidth != currentOutputWidth ||
      sourceHeight != currentOutputHeight || offset ||
      globalScaleRatio != 1.0f) {
    calc_scale_factor(currentScaleRatio_x, currentScaleRatio_y, sourceWidth,
                      sourceHeight);

    drawXOffset =
        ((int)currentOutputWidth - (int)sourceWidth * currentScaleRatio_x) /
        2.0f;
    drawYOffset =
        ((int)currentOutputHeight - (int)sourceHeight * currentScaleRatio_y) /
        2.0f;
    ...
    if (zoomScaleRatio != 1.0) {
      drawXOffset +=
          (((int)sourceWidth / 2) - cursor->x()) * currentScaleRatio_x;
      drawYOffset +=
          (((int)sourceHeight / 2) - cursor->y()) * currentScaleRatio_y;
    }
  }
  ...
  layer->opacity =
      ((w->isOverlay || w->isExternalOverlay) ? w->opacity / (float)OPAQUE
                                              : 1.0f) *
      flOpacityScale;
  ...
  if (w != scaleW) {
    ...
    if (globalScaleRatio != 1.0f) {
      xOffset =
          (currentOutputWidth - currentOutputWidth * globalScaleRatio) / 2.0;
      yOffset =
          (currentOutputHeight - currentOutputHeight * globalScaleRatio) / 2.0;
    }
...
static void paint_all(bool async) {
  ...
  bool fadingOut =
      (currentTime - fadeOutStartTime < g_FadeOutDuration || g_bPendingFade) &&
  ...
  if (w) {
    if (w->isSteamStreamingClient == true) {
      ...
      for (size_t i = 0; (server = wlserver_get_xwayland_server(i)); i++) {
        for (videow = server->ctx->list; videow;
          ...
          if (videow->isSteamStreamingClientVideo == true) {
            ...
            paint_window(videow, videow, &frameInfo, global_focus.cursor,
      ...
      // paint UI unless it's fully hidden, which it communicates to us through
      // opacity=0 we paint it to extract scaling coefficients above, then
      // remove the layer if one was added
      ...
      if (fadingOut) {
        ...
        paint_cached_base_layer(g_HeldCommits[HELD_COMMIT_FADE],
                                g_CachedPlanes[HELD_COMMIT_FADE], &frameInfo,
                                1.0f - opacityScale);
        paint_window(w, w, &frameInfo, global_focus.cursor,
                     PaintWindowFlag::BasePlane | PaintWindowFlag::FadeTarget |
                         PaintWindowFlag::DrawBorders,
                     opacityScale, override);
      } else {
        {
        ...
        paint_window(w, w, &frameInfo, global_focus.cursor,
                     PaintWindowFlag::BasePlane | PaintWindowFlag::DrawBorders,
                     1.0f, override);

        bool needsScaling = frameInfo.layers[0].scale.x < 0.999f &&
                            frameInfo.layers[0].scale.y < 0.999f;
        frameInfo.useFSRLayer0 =
            g_upscaleFilter == GamescopeUpscaleFilter::FSR && needsScaling;
        frameInfo.useNISLayer0 =
            g_upscaleFilter == GamescopeUpscaleFilter::NIS && needsScaling;
      }
      update_touch_scaling(&frameInfo);
    }
  } else {
  ...
  if (override && w && !w->isSteamStreamingClient) {
    paint_window(override, w, &frameInfo, global_focus.cursor, 0, 1.0f,
                 override);
  ...
  if (externalOverlay) {
    if (externalOverlay->opacity) {
      paint_window(externalOverlay, externalOverlay, &frameInfo,
                   global_focus.cursor, PaintWindowFlag::NoScale);
  ...
  if (overlay) {
    if (overlay->opacity) {
      paint_window(overlay, overlay, &frameInfo, global_focus.cursor,
                   PaintWindowFlag::DrawBorders);
  ...
  if (notification) {
    if (notification->opacity) {
      paint_window(notification, notification, &frameInfo, global_focus.cursor,
                   PaintWindowFlag::NotificationMode);
    }
  }
  ...
  if (input && !bForceHideCursor) {
    ...
    global_focus.cursor->paint(input, w == input ? override : nullptr,
                               &frameInfo);
  ...
  int nTargetRefresh =
      nDynamicRefresh &&
              steamcompmgr_window_should_limit_fps(
                  global_focus.focusWindow) // && !global_focus.overlayWindow
          ? nDynamicRefresh
  ...
  if (!BIsNested() && g_nOutputRefresh != nTargetRefresh &&
      g_uDynamicRefreshEqualityTime + g_uDynamicRefreshDelay < now)
    drm_set_refresh(&g_DRM, nTargetRefresh);

  bool bNeedsNearest = g_upscaleFilter == GamescopeUpscaleFilter::NEAREST &&
                       frameInfo.layers[0].scale.x != 1.0f &&
                       frameInfo.layers[0].scale.y != 1.0f;
  ...
  bool bWantsPartialComposite =
      frameInfo.layerCount >= 3 && !kDisablePartialComposition;
  ...
  if (!BIsNested() && g_bOutputHDREnabled) {
    ...
    if (!drm_supports_color_mgmt(&g_DRM))
      bNeedsFullComposite |= (frameInfo.layerCount > 1 ||
  ...
  if (bDoComposite == true) {
    ...
    if (!bNeedsFullComposite) {
      ...
      // This is 2, and we do i-1 so 1...layerCount. So AFTER we have removed
      // baseplane. Overlays only.
      //
      // Josh:
      ...
      // We can't just point it to random BDA or whatever, it has to be uploaded
      // slowly thru registers which is SUPER SLOW. This avoids stutter.
      for (int i = 2; i < compositeFrameInfo.layerCount; i++) {
        if (frameInfo.layers[i - 1].colorspace !=
    ...
    // We were already stalling for the full composition before, so it's not an
    // issue for latency, we just need to make sure we get 1 partial frame that
    // isn't deferred in time so we don't lose layers.
    bool bDefer =
        !bNeedsFullComposite && (!g_bWasCompositing || g_bWasPartialComposite);
    ...
    if (BIsNested() == true) {
      ...
      if (bNeedsFullComposite) {
        ...
        baseLayer->colorspace = g_bOutputHDREnabled
                                    ? GAMESCOPE_APP_TEXTURE_COLORSPACE_HDR10_PQ
      ...
      int32_t width = presentCompFrameInfo.layers[0].tex.get()->contentWidth();
      int32_t height =
      ...
      if (ret != 0) {
        if (g_DRM.current.mode_id == 0) {
          xwm_log.errorf("We failed our modeset and have no mode to fall back "
                         "to! (Initial modeset failed?): %s",
                         strerror(-ret));
          abort();
        }

        xwm_log.errorf("Failed to prepare 1-layer flip (%s), trying again with "
  ...
  if (takeScreenshot) {
    ...
    uint32_t drmCaptureFormat =
        bHackForceNV12DumpScreenshot ? DRM_FORMAT_NV12 : DRM_FORMAT_XRGB8888;

    std::shared_ptr<CVulkanTexture> pScreenshotTexture =
        vulkan_acquire_screenshot_texture(g_nOutputWidth, g_nOutputHeight,
                                          false, drmCaptureFormat);

    assert(pScreenshotTexture != nullptr);

    // Basically no color mgmt applied for screenshots. (aside from being able
    // to handle HDR content with LUTs)
    for (uint32_t nInputEOTF = 0; nInputEOTF < EOTF_Count; nInputEOTF++) {
    ...
    std::thread screenshotThread = std::thread([=] {
      ...
      if (pScreenshotTexture->format() == VK_FORMAT_B8G8R8A8_UNORM) {
        ...
        auto imageData =
            std::vector<uint8_t>(currentOutputWidth * currentOutputHeight * 4);
        ...
        for (uint32_t y = 0; y < currentOutputHeight; y++) {
          for (uint32_t x = 0; x < currentOutputWidth; x++) {
            ...
            imageData[y * pitch + x * comp + 0] =
                mappedData[y * pScreenshotTexture->rowPitch() + x * comp + 2];
            imageData[y * pitch + x * comp + 1] =
                mappedData[y * pScreenshotTexture->rowPitch() + x * comp + 1];
            imageData[y * pitch + x * comp + 2] =
                mappedData[y * pScreenshotTexture->rowPitch() + x * comp + 0];
        ...
        if (!propertyRequestedScreenshot) {
          ...
          strftime(pTimeBuffer, sizeof(pTimeBuffer),
                   "/tmp/gamescope_%Y-%m-%d_%H-%M-%S.png", localTime);
        }

        if (stbi_write_png(pTimeBuffer, currentOutputWidth, currentOutputHeight,
                           4, imageData.data(), pitch)) {
          xwm_log.infof("Screenshot saved to %s", pTimeBuffer);
        } else {
          xwm_log.errorf("Failed to save screenshot to %s", pTimeBuffer);
        }
      } else if (pScreenshotTexture->format() ==
        ...
        if (!propertyRequestedScreenshot) {
          ...
          struct tm *localTime = localtime(&currentTime);
          strftime(pTimeBuffer, sizeof(pTimeBuffer),
                   "/tmp/gamescope_%Y-%m-%d_%H-%M-%S.raw", localTime);
        }
        ...
        if (file) {
          ...
          char cmd[4096];
          sprintf(cmd,
                  "ffmpeg -f rawvideo -pixel_format nv12 -video_size %dx%d -i "
                  "%s %s_encoded.png",
                  pScreenshotTexture->width(), pScreenshotTexture->height(),
                  pTimeBuffer, pTimeBuffer);
      ...
      XDeleteProperty(root_ctx->dpy, root_ctx->root,
  ...
  gpuvis_trace_printf("paint_all %i layers, composite %i",
                      (int)frameInfo.layerCount, bDoComposite);
}
 ...
 *   otherwise the value
 */
__attribute__((__no_sanitize_address__)) // x11 broken, returns format 32 even
                                         // when it only malloc'ed one byte. :(
static unsigned int
get_prop(xwayland_ctx_t *ctx, Window win, Atom prop, unsigned int def,
         bool *found = nullptr) {
  Atom actual;
  int format;
  ...
  unsigned char *data;
  int result =
      XGetWindowProperty(ctx->dpy, win, prop, 0L, 1L, false, XA_CARDINAL,
...
bool get_prop( xwayland_ctx_t *ctx, Window win, Atom prop, std::vector< uint32_t > &vecResult )
{
  ...
  uint64_t *data;
  int result =
      XGetWindowProperty(ctx->dpy, win, prop, 0L, ~0UL, false, XA_CARDINAL,
...
static bool win_maybe_a_dropdown(steamcompmgr_win_t *w) {
  ...
  if (w->appID == 230410 && w->maybe_a_dropdown && w->xwayland().transientFor &&
  ...
  // The Launcher in Witcher 2 (20920) has a clear window with WS_EX_LAYERED on
  // top of it.
  //
  ...
  // TOOLWINDOW, and we don't want that to be overlayed. Ignore LAYERED if it's
  // marked as top-level with WS_EX_APPWINDOW.
  ...
  // 		- If the window has transient for, disregard it, as it is trying
  // to redirecting us elsewhere
  ...
  bool valid_maybe_a_dropdown =
      w->maybe_a_dropdown &&
      ((!w->is_dialog ||
        (!w->xwayland().transientFor && win_skip_and_not_fullscreen(w))) &&
       (w->skipPager || w->skipTaskbar));
 ...
 */
static bool is_focus_priority_greater(steamcompmgr_win_t *a,
                                      steamcompmgr_win_t *b) {
...
static bool is_good_override_candidate(steamcompmgr_win_t *override,
                                       steamcompmgr_win_t *focus) {
  ...
  return override != focus && override->xwayland().a.x >= 0 &&
         override->xwayland().a.y >= 0;
}

static bool pick_primary_focus_and_override(
    focus_t *out, Window focusControlWindow,
    const std::vector<steamcompmgr_win_t *> &vecPossibleFocusWindows,
    bool globalFocus, const std::vector<uint32_t> &ctxFocusControlAppIDs) {
  ...
  auto resolveTransientOverrides = [&](bool maybe) {
    ...
    while (true) {
      ...
      for (steamcompmgr_win_t *candidate : vecPossibleFocusWindows) {
        ...
        bool is_dropdown = maybe ? win_maybe_a_dropdown(candidate)
                                 : win_is_override_redirect(candidate);
        if ((!override_focus || candidate != override_focus) &&
            candidate != focus &&
            ((!override_focus &&
              candidate->xwayland().transientFor == focus->xwayland().id) ||
             (override_focus && candidate->xwayland().transientFor ==
      ...
      // Hopefully we can't have transient cycles or we'll have to maintain a
      // list of visited windows here
  ...
  if (focus && focus->type == steamcompmgr_win_type_t::XWAYLAND) {
    if (!focusControlWindow) {
      // Do some searches through game windows to follow transient links if
      // needed
      while (true) {
        bool bFoundTransient = false;
...
static void determine_and_apply_focus(
    xwayland_ctx_t *ctx,
    std::vector<steamcompmgr_win_t *> &vecGlobalPossibleFocusWindows) {
  steamcompmgr_win_t *w;
  ...
  for (w = ctx->list; w; w = w->xwayland().next) {
    ...
    if (w->xwayland().a.map_state == IsViewable &&
        w->xwayland().a.c_class == InputOutput && w->isOverlay == false &&
        w->isExternalOverlay == false &&
        (win_has_game_id(w) || window_is_steam(w) ||
  ...
  std::stable_sort(vecPossibleFocusWindows.begin(),
                   vecPossibleFocusWindows.end(), is_focus_priority_greater);

  vecGlobalPossibleFocusWindows.insert(vecGlobalPossibleFocusWindows.end(),
                                       vecPossibleFocusWindows.begin(),
                                       vecPossibleFocusWindows.end());

  pick_primary_focus_and_override(&ctx->focus, ctx->focusControlWindow,
                                  vecPossibleFocusWindows, false,
  ...
  if (prevFocusWindow != ctx->focus.focusWindow) {
    ...
    XChangeProperty(ctx->dpy, ctx->focus.focusWindow->xwayland().id,
                    ctx->atoms.WMStateAtom, ctx->atoms.WMStateAtom, 32,
    ...
    if (debugFocus == true) {
      xwm_log.debugf("determine_and_apply_focus focus %lu",
                     ctx->focus.focusWindow->xwayland().id);
      char buf[512];
      sprintf(buf, "xwininfo -id 0x%lx; xprop -id 0x%lx; xwininfo -root -tree",
              ctx->focus.focusWindow->xwayland().id,
              ctx->focus.focusWindow->xwayland().id);
      system(buf);
    }
  ...
  if (ctx->focus.inputFocusWindow != inputFocus ||
    ...
    if (debugFocus == true) {
      xwm_log.debugf("determine_and_apply_focus inputFocus %lu",
                     inputFocus->xwayland().id);
    }

    if (!ctx->focus.overrideWindow ||
        ctx->focus.overrideWindow != keyboardFocusWin)
      XSetInputFocus(ctx->dpy, keyboardFocusWin->xwayland().id, RevertToNone,
                     CurrentTime);
    ...
    if (ctx->focus.inputFocusWindow != inputFocus ||
      ...
      if (window_wants_no_focus_when_mouse_hidden(inputFocus))
        bResetToCorner = true;
      else if (window_wants_no_focus_when_mouse_hidden(inputFocus) !=
      ...
      // going from override -> focus and we don't want to hide then as it's
      // probably a dropdown.
      ctx->cursor->hide();
    }
  ...
  if (inputFocus == ctx->focus.focusWindow && ctx->focus.overrideWindow) {
    if (ctx->list[0].xwayland().id !=
        ctx->focus.overrideWindow->xwayland().id) {
  ...
  if (window_is_fullscreen(ctx->focus.focusWindow) ||
    ...
    if (w->xwayland().a.width != fs_width ||
        w->xwayland().a.height != fs_height || globalScaleRatio != 1.0f)
      XResizeWindow(ctx->dpy, ctx->focus.focusWindow->xwayland().id, fs_width,
                    fs_height);
  } else {
    if (ctx->focus.focusWindow->sizeHintsSpecified &&
        ((unsigned)ctx->focus.focusWindow->xwayland().a.width !=
             ctx->focus.focusWindow->requestedWidth ||
         (unsigned)ctx->focus.focusWindow->xwayland().a.height !=
             ctx->focus.focusWindow->requestedHeight)) {
      XResizeWindow(ctx->dpy, ctx->focus.focusWindow->xwayland().id,
                    ctx->focus.focusWindow->requestedWidth,
                    ctx->focus.focusWindow->requestedHeight);
    }
  }
  ...
  if (inputFocus) {
    ...
    if (bResetToCorner) {
      ...
      ctx->cursor->forcePosition(inputFocus->xwayland().a.width - 1,
                                 inputFocus->xwayland().a.height - 1);
    } else if (bResetToCenter) {
      inputFocus->mouseMoved = 0;
      ctx->cursor->forcePosition(inputFocus->xwayland().a.width / 2,
                                 inputFocus->xwayland().a.height / 2);
    }
  }
  ...
  XQueryTree(ctx->dpy, w->xwayland().id, &root_return, &parent_return,
             &children, &nchildren);
...
static void determine_and_apply_focus() {
  ...
  for (steamcompmgr_win_t *focusable_window : vecPossibleFocusWindows) {
    ...
    // Exclude windows that are useless (1x1), skip taskbar + pager or override
    // redirect windows from the reported focusable windows to Steam.
  ...
  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeFocusableAppsAtom, XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)focusable_appids.data(),
                  focusable_appids.size());

  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeFocusableWindowsAtom, XA_CARDINAL,
                  32, PropModeReplace,
  ...
  gameFocused = pick_primary_focus_and_override(
      &global_focus, root_ctx->focusControlWindow, vecPossibleFocusWindows,
      true, vecFocuscontrolAppIDs);
  ...
  if (global_focus.overlayWindow &&
      global_focus.overlayWindow->inputFocusMode) {
  ...
  if (global_focus.inputFocusWindow) {
    if (global_focus.inputFocusWindow->type ==
  ...
  if (global_focus.inputFocusWindow != previous_focus.inputFocusWindow ||
    ...
    if (global_focus.cursor &&
        global_focus.inputFocusWindow != previous_focus.inputFocusWindow)
      global_focus.cursor->hide();
  }

  // Determine if we need to repaints
  if (previous_focus.overlayWindow != global_focus.overlayWindow ||
      previous_focus.externalOverlayWindow !=
          global_focus.externalOverlayWindow ||
  ...
  if (global_focus.keyboardFocusWindow) {
    focused_keyboard_display =
        get_win_display_name(global_focus.keyboardFocusWindow);
  }

  if (steamMode) {
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeFocusedAppAtom, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&focusedAppId,
                    focusedAppId != 0 ? 1 : 0);

    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeFocusedAppGfxAtom, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&focusedBaseAppId,
                    focusedBaseAppId != 0 ? 1 : 0);
  }

  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeFocusedWindowAtom, XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)&focusedWindow,
                  focusedWindow != 0 ? 1 : 0);

  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeFocusDisplay, XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)focused_display,
                  strlen(focused_display) + 1);

  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeMouseFocusDisplay, XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)focused_mouse_display,
                  strlen(focused_mouse_display) + 1);

  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeKeyboardFocusDisplay, XA_CARDINAL,
                  32, PropModeReplace,
  ...
  if (previous_focus.focusWindow != global_focus.focusWindow) {
    if (g_FadeOutDuration != 0 && !g_bFirstFrame) {
      if (!g_HeldCommits[HELD_COMMIT_FADE]) {
        ...
        // If we end up fading back to what we were going to fade to, cancel the
        // fade.
        if (global_focus.fadeWindow != nullptr &&
            global_focus.focusWindow == global_focus.fadeWindow) {
          g_HeldCommits[HELD_COMMIT_FADE] = nullptr;
          g_bPendingFade = false;
          fadeOutStartTime = 0;
  ...
  if (global_focus.focusWindow &&
    ...
    get_window_last_done_commit(global_focus.focusWindow,
  ...
  if (global_focus.inputFocusWindow &&
      global_focus.inputFocusWindow->appID == 405900) {
...
static void get_size_hints(xwayland_ctx_t *ctx, steamcompmgr_win_t *w) {
  ...
  const bool bHasPositionAndGravityHints =
      (hintsSpecified & (PPosition | PWinGravity)) == (PPosition | PWinGravity);
  if (bHasPositionAndGravityHints && hints.x && hints.y &&
...
static void get_motif_hints(xwayland_ctx_t *ctx, steamcompmgr_win_t *w) {
  if (w->motif_hints)
    XFree(w->motif_hints);
  ...
  XGetWindowProperty(ctx->dpy, w->xwayland().id, ctx->atoms.motifWMHints, 0L,
                     20L, False, ctx->atoms.motifWMHints, &actual_type,
                     &actual_format, &nitems, &bytesafter,
                     (unsigned char **)&w->motif_hints);
}

static void get_win_title(xwayland_ctx_t *ctx, steamcompmgr_win_t *w,
                          Atom atom) {
...
static void get_net_wm_state(xwayland_ctx_t *ctx, steamcompmgr_win_t *w) {
  ...
  for (size_t i = 0; i < nitems; i++) {
    if (props[i] == ctx->atoms.netWMStateFullscreenAtom) {
      ...
      xwm_log.debugf("Unhandled initial NET_WM_STATE property: %s",
...
static void map_win(xwayland_ctx_t *ctx, Window id, unsigned long sequence) {
  ...
  w->isSteamStreamingClient =
      get_prop(ctx, w->xwayland().id, ctx->atoms.steamStreamingClientAtom, 0);
  ...
  if (wmHints != nullptr) {
    if (wmHints->flags & (InputHint | StateHint) && wmHints->input == true &&
...
static uint32_t get_appid_from_pid(pid_t pid) {
      ...
      for (uint32_t j = 0; j < proc_cmdline.length(); j++) {
        if (proc_cmdline[j] == '\0' && j + 1 < proc_cmdline.length()) {
          if (strcmp("SteamLaunch", &proc_cmdline[j + 1]) == 0) {
            bSteamLaunch = true;
          } else if (sscanf(&proc_cmdline[j + 1], "AppId=%u", &unAppId) == 1 &&
                     unAppId != 0) {
...
static void add_win(xwayland_ctx_t *ctx, Window id, Window prev,
                    unsigned long sequence) {
  ...
  wlserver_x11_surface_info_init(&new_win->xwayland().surface,
                                 ctx->xwayland_server, id);
...
static void restack_win(xwayland_ctx_t *ctx, steamcompmgr_win_t *w,
                        Window new_above) {
  Window old_above;
...
static void configure_request(xwayland_ctx_t *ctx,
  ...
  XConfigureWindow(ctx->dpy, configureRequest->window,
                   configureRequest->value_mask, &changes);
}

static void circulate_request(xwayland_ctx_t *ctx,
                              XCirculateRequestEvent *circulateRequest) {
  XCirculateSubwindows(ctx->dpy, circulateRequest->window,
                       circulateRequest->place);
}
...
static void damage_win(xwayland_ctx_t *ctx, XDamageNotifyEvent *de) {
  ...
  gpuvis_trace_printf("damage_win win %lx appID %u", w->xwayland().id,
                      w->appID);
}

static void handle_wl_surface_id(xwayland_ctx_t *ctx, steamcompmgr_win_t *w,
                                 uint32_t surfaceID) {
...
static void handle_net_wm_state(xwayland_ctx_t *ctx, steamcompmgr_win_t *w,
  ...
  for (size_t i = 0; i < 2; i++) {
    if (props[i] == ctx->atoms.netWMStateFullscreenAtom) {
      ...
      xwm_log.debugf("Unhandled NET_WM_STATE property change: %s",
                     XGetAtomName(ctx->dpy, props[i]));
    }
  }
}
...
static void handle_system_tray_opcode(xwayland_ctx_t *ctx,
                                      XClientMessageEvent *ev) {
...
static void handle_wm_change_state(xwayland_ctx_t *ctx, steamcompmgr_win_t *w,
                                   XClientMessageEvent *ev) {
  ...
  if (state == ICCCM_ICONIC_STATE) {
    ...
    xwm_log.debugf("Rejecting WM_CHANGE_STATE to ICONIC for window 0x%lx",
                   w->xwayland().id);
    uint32_t wmState[] = {ICCCM_NORMAL_STATE, None};
    XChangeProperty(ctx->dpy, w->xwayland().id, ctx->atoms.WMStateAtom,
                    ctx->atoms.WMStateAtom, 32, PropModeReplace,
                    (unsigned char *)wmState,
                    sizeof(wmState) / sizeof(wmState[0]));
  } else {
    xwm_log.debugf("Unhandled WM_CHANGE_STATE to %ld for window 0x%lx", state,
                   w->xwayland().id);
  }
}

static void handle_client_message(xwayland_ctx_t *ctx,
                                  XClientMessageEvent *ev) {
  ...
  if (w) {
    if (ev->message_type == ctx->atoms.WLSurfaceIDAtom) {
      ...
      xwm_log.debugf("Unhandled client message: %s",
...
static void steamcompmgr_latch_frame_done(steamcompmgr_win_t *w,
                                          uint64_t vblank_idx) {
  ...
  if (g_nSteamCompMgrTargetFPS && steamcompmgr_window_should_limit_fps(w) &&
      nRefresh > nTargetFPS) {
...
static void handle_property_notify(xwayland_ctx_t *ctx, XPropertyEvent *ev) {
  ...
  if (ev->atom == ctx->atoms.steamTouchClickModeAtom) {
    g_nTouchClickMode = (enum wlserver_touch_click_mode)get_prop(
        ctx, ctx->root, ctx->atoms.steamTouchClickModeAtom,
        g_nDefaultTouchClickMode);
#if HAVE_OPENVR
    if (BIsVRSession())
  ...
  if (ev->atom == ctx->atoms.gamescopeCtrlAppIDAtom) {
    get_prop(ctx, ctx->root, ctx->atoms.gamescopeCtrlAppIDAtom,
             vecFocuscontrolAppIDs);
    focusDirty = true;
  }
  if (ev->atom == ctx->atoms.gamescopeCtrlWindowAtom) {
    ctx->focusControlWindow =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeCtrlWindowAtom, None);
    focusDirty = true;
  }
  ...
  if (ev->atom == ctx->atoms.gamescopeTuneableVBlankRedZone) {
    g_uVblankDrawBufferRedZoneNS = (uint64_t)get_prop(
        ctx, ctx->root, ctx->atoms.gamescopeTuneableVBlankRedZone,
        g_uDefaultVBlankRedZone);
  }
  if (ev->atom == ctx->atoms.gamescopeTuneableRateOfDecay) {
    g_uVBlankRateOfDecayPercentage = (uint64_t)get_prop(
        ctx, ctx->root, ctx->atoms.gamescopeTuneableRateOfDecay,
        g_uDefaultVBlankRateOfDecayPercentage);
  }
  ...
  if (ev->atom == ctx->atoms.gamescopeXWaylandModeControl) {
    ...
    bool hasModeCtrl =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeXWaylandModeControl,
                 xwayland_mode_ctl);
    if (hasModeCtrl && xwayland_mode_ctl.size() == 4) {
      ...
      if (server) {
        ...
        wlserver_lock();
        wlserver_set_xwayland_server_mode(server_idx, width, height,
                                          g_nOutputRefresh);
        wlserver_unlock();
  ...
  for (int i = 0; i < DRM_SCREEN_TYPE_COUNT; i++) {
    if (ev->atom == ctx->atoms.gamescopeDynamicRefresh[i]) {
      g_nDynamicRefreshRate[i] =
          get_prop(ctx, ctx->root, ctx->atoms.gamescopeDynamicRefresh[i], 0);
    }
  }
  ...
  if (ev->atom == ctx->atoms.gamescopeBlurMode) {
    BlurMode newBlur =
        (BlurMode)get_prop(ctx, ctx->root, ctx->atoms.gamescopeBlurMode, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeDisplayModeNudge) {
    if (!BIsNested()) {
      g_DRM.out_of_date = 2;
      XDeleteProperty(ctx->dpy, ctx->root,
                      ctx->atoms.gamescopeDisplayModeNudge);
    }
  }
  if (ev->atom == ctx->atoms.gamescopeNewScalingFilter) {
    GamescopeUpscaleFilter nScalingFilter = (GamescopeUpscaleFilter)get_prop(
  ...
  if (ev->atom == ctx->atoms.gamescopeNewScalingScaler) {
    GamescopeUpscaleScaler nScalingScaler = (GamescopeUpscaleScaler)get_prop(
  ...
  if (ev->atom == ctx->atoms.gamescopeDisplayHDREnabled) {
    g_bHDREnabled =
        !!get_prop(ctx, ctx->root, ctx->atoms.gamescopeDisplayHDREnabled, 0);
    hasRepaint = true;
  }
  ...
  if (ev->atom == ctx->atoms.gamescopeDebugForceHDRSupport) {
    g_bForceHDRSupportDebug =
        !!get_prop(ctx, ctx->root, ctx->atoms.gamescopeDebugForceHDRSupport, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeHDRTonemapDisplayMetadata) {
    ...
    if (get_prop(ctx, ctx->root, ctx->atoms.gamescopeHDRTonemapDisplayMetadata,
                 user_vec) &&
        user_vec.size() >= 2) {
  ...
  if (ev->atom == ctx->atoms.gamescopeHDRTonemapSourceMetadata) {
    std::vector<uint32_t> user_vec;
    if (get_prop(ctx, ctx->root, ctx->atoms.gamescopeHDRTonemapSourceMetadata,
                 user_vec) &&
        user_vec.size() >= 2) {
  ...
  if (ev->atom == ctx->atoms.gamescopeSDROnHDRContentBrightness) {
    uint32_t val = get_prop(ctx, ctx->root,
                            ctx->atoms.gamescopeSDROnHDRContentBrightness, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeInternalDisplayBrightness) {
    uint32_t val = get_prop(ctx, ctx->root,
                            ctx->atoms.gamescopeInternalDisplayBrightness, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeColorLut3DOverride) {
    std::string path =
        get_string_prop(ctx, ctx->root, ctx->atoms.gamescopeColorLut3DOverride);
  ...
  if (ev->atom == ctx->atoms.gamescopeColorSDRGamutWideness) {
    uint32_t val =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeColorSDRGamutWideness, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeColorNightMode) {
    std::vector<uint32_t> user_vec;
    bool bHasVec =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeColorNightMode, user_vec);
  ...
  if (ev->atom == ctx->atoms.gamescopeColorManagementDisable) {
    uint32_t val =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeColorManagementDisable, 0);
  ...
  if (ev->atom == ctx->atoms.gamescopeCreateXWaylandServer) {
    uint32_t identifier =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeCreateXWaylandServer, 0);
    if (identifier) {
      wlserver_lock();
      ...
      snprintf(propertyString, sizeof(propertyString), "%u %u %s", identifier,
               server_id, server->get_nested_display_name());
      ...
      XSetTextProperty(ctx->dpy, ctx->root, &text_property,
  ...
  if (ev->atom == ctx->atoms.gamescopeDestroyXWaylandServer) {
    uint32_t server_id =
        get_prop(ctx, ctx->root, ctx->atoms.gamescopeDestroyXWaylandServer, 0);
  ...
  if (ev->atom == ctx->atoms.wineHwndStyleEx) {
    ...
    if (w) {
      ...
      w->hwndStyleEx =
          get_prop(ctx, w->xwayland().id, ctx->atoms.wineHwndStyleEx, 0);
      focusDirty = true;
    }
  }
}
...
static int error(Display *dpy, XErrorEvent *ev) {
  ...
  xwm_log.errorf("error %d: %s request %d minor %d serial %lu", ev->error_code,
                 (strlen(name) > 0) ? name : "unknown", ev->request_code,
...
[[noreturn]] static void steamcompmgr_exit(void) {
...
static int handle_io_error(Display *dpy) {
...
static bool register_cm(xwayland_ctx_t *ctx) {
  ...
  if (w != None) {
    ...
    if (!XGetTextProperty(ctx->dpy, w, &tp, winNameAtom) &&
      ...
      xwm_log.errorf("Another composite manager is already running (0x%lx)",
                     (unsigned long)w);
      return false;
    }
    if (XmbTextPropertyToTextList(ctx->dpy, &tp, &strs, &count) == Success) {
      xwm_log.errorf("Another composite manager is already running (%s)",
                     strs[0]);
  ...
  w = XCreateSimpleWindow(ctx->dpy, RootWindow(ctx->dpy, ctx->scr), 0, 0, 1, 1,
                          0, None, None);

  Xutf8SetWMProperties(ctx->dpy, w, "steamcompmgr", "steamcompmgr", NULL, 0,
                       NULL, NULL, NULL);

  Atom atomWmCheck = XInternAtom(ctx->dpy, "_NET_SUPPORTING_WM_CHECK", false);
  XChangeProperty(ctx->dpy, ctx->root, atomWmCheck, XA_WINDOW, 32,
                  PropModeReplace, (unsigned char *)&w, 1);
  XChangeProperty(ctx->dpy, w, atomWmCheck, XA_WINDOW, 32, PropModeReplace,
                  (unsigned char *)&w, 1);
  ...
  XChangeProperty(ctx->dpy, ctx->root,
                  XInternAtom(ctx->dpy, "_NET_SUPPORTED", false), XA_ATOM, 32,
                  PropModeAppend, (unsigned char *)supportedAtoms,
...
bool handle_done_commit(steamcompmgr_win_t *w, xwayland_ctx_t *ctx,
                        uint64_t commitID) {
  ...
  for (j = 0; j < w->commit_queue.size(); j++) {
    if (w->commit_queue[j]->commitID == commitID) {
      ...
      // Window just got a new available commit, determine if that's worth a
      // repaint
      ...
      if (global_focus.outdatedInteractiveFocus) {
        ...
        if (w == global_focus.externalOverlayWindow &&
      ...
      if (w->isSteamStreamingClientVideo && global_focus.focusWindow &&
          global_focus.focusWindow->isSteamStreamingClient) {
        g_HeldCommits[HELD_COMMIT_BASE] = w->commit_queue[j];
        hasRepaint = true;
      }
  ...
  if (bFoundWindow == true) {
    if (j > 0) {
      ...
      w->commit_queue.erase(w->commit_queue.begin(),
                            w->commit_queue.begin() + j);
    }
...
void handle_done_commits_xdg() {
  ...
  // very fast loop yes
  for (uint32_t i = 0;
       i < g_steamcompmgr_xdg_done_commits.listCommitsDone.size(); i++) {
    for (const auto &xdg_win : g_steamcompmgr_xdg_wins) {
      if (handle_done_commit(
              xdg_win.get(), nullptr,
              g_steamcompmgr_xdg_done_commits.listCommitsDone[i]))
        break;
    }
  }
...
void handle_presented_for_window(steamcompmgr_win_t *w) {
  ...
  if (lastCommit && !lastCommit->presentation_feedbacks.empty()) {
    ...
    wlserver_presentation_feedback_presented(lastCommit->surf,
                                             lastCommit->presentation_feedbacks,
                                             g_lastVblank.load(), nRefresh);
...
void update_wayland_res(CommitDoneList_t *doneCommits, steamcompmgr_win_t *w,
  ...
  std::shared_ptr<commit_t> newCommit =
      import_commit(reslistentry.surf, buf, reslistentry.async,
  ...
  if (newCommit) {
    ...
    const bool mango_nudge =
        (w == global_focus.focusWindow && !w->isSteamStreamingClient) ||
        (global_focus.focusWindow &&
         global_focus.focusWindow->isSteamStreamingClient &&
         w->isSteamStreamingClientVideo);

    gpuvis_trace_printf(
        "pushing wait for commit %lu win %lx", newCommit->commitID,
        w->type == steamcompmgr_win_type_t::XWAYLAND ? w->xwayland().id : 0);
    {
      std::unique_lock<std::mutex> lock(waitListLock);
      WaitListEntry_t entry{
  ...
  for (uint32_t i = 0; i < tmp_queue.size(); i++) {
    for (const auto &xdg_win : g_steamcompmgr_xdg_wins) {
      if (xdg_win->xdg().surface.main_surface == tmp_queue[i].surf) {
        update_wayland_res(&g_steamcompmgr_xdg_done_commits, xdg_win.get(),
                           tmp_queue[i]);
        break;
      }
    }
...
#warning                                                                       \
  ...
  if (pchCurrentPreload != nullptr) {
    ...
    while (i < strlen(pchCurrentPreload)) {
      // If there's a string and it's not gameoverlayrenderer, append it to our
      // new LD_PRELOAD
...
static void dispatch_x11(xwayland_ctx_t *ctx) {
  ...
  while (XPending(ctx->dpy)) {
    ...
    case FocusOut: {
      ...
      // If focus escaped the current desired keyboard focus window, check where
      // it went
      if (w && w->xwayland().id == ctx->currentKeyboardFocusWindow) {
        ...
        if (kbw) {
          if (kbw->xwayland().id == ctx->currentKeyboardFocusWindow) {
            // focus went to a child, this is fine, make note of it in case we
            // need to fix it
  ...
  if (bSetFocus) {
    XSetInputFocus(ctx->dpy, ctx->currentKeyboardFocusWindow, RevertToNone,
                   CurrentTime);
  }
}

static bool dispatch_vblank(int fd) {
  bool vblank = false;
  for (;;) {
...
static void dispatch_nudge(int fd) {
  for (;;) {
...
static bool load_mouse_cursor(MouseCursor *cursor, const char *path, int hx,
                              int hy) {
  int w, h, channels;
  ...
  std::transform(data, data + w * h, data, [](rgba_t x) {
    if (x.a == 0)
      return rgba_t{};
    return rgba_t{uint8_t((x.b * x.a) / 255), uint8_t((x.g * x.a) / 255),
                  uint8_t((x.r * x.a) / 255), x.a};
  });
...
static bool load_host_cursor(MouseCursor *cursor) {
  ...
  cursor->setCursorImage((char *)cursorData.data(), image->width, image->height,
                         image->xhot, image->yhot);
  return true;
}
...
void init_xwayland_ctx(uint32_t serverId,
  ...
  if (!XRenderQueryExtension(ctx->dpy, &ctx->render_event,
                             &ctx->render_error)) {
  ...
  if (!XDamageQueryExtension(ctx->dpy, &ctx->damage_event,
                             &ctx->damage_error)) {
  ...
  if (xres_major != 1 || xres_minor < 2) {
    xwm_log.errorf("Unsupported XRes version: have %d.%d, want 1.2", xres_major,
                   xres_minor);
    exit(1);
  }
  ...
  ctx->atoms.gamescopeDynamicRefresh[DRM_SCREEN_TYPE_INTERNAL] =
      XInternAtom(ctx->dpy, "GAMESCOPE_DYNAMIC_REFRESH", false);
  ctx->atoms.gamescopeDynamicRefresh[DRM_SCREEN_TYPE_EXTERNAL] =
  ...
  ctx->atoms.gamescopeSDROnHDRContentBrightness =
      XInternAtom(ctx->dpy, "GAMESCOPE_SDR_ON_HDR_CONTENT_BRIGHTNESS", false);
  ctx->atoms.gamescopeInternalDisplayBrightness =
      XInternAtom(ctx->dpy, "GAMESCOPE_INTERNAL_DISPLAY_BRIGHTNESS", false);
  ...
  ctx->atoms.gamescopeHDRTonemapDisplayMetadata =
      XInternAtom(ctx->dpy, "GAMESCOPE_HDR_TONEMAP_DISPLAY_METADATA", false);
  ...
  ctx->atoms.gamescopeColorAppWantsHDRFeedback =
      XInternAtom(ctx->dpy, "GAMESCOPE_COLOR_APP_WANTS_HDR_FEEDBACK", false);
  ctx->atoms.gamescopeColorAppHDRMetadataFeedback =
      XInternAtom(ctx->dpy, "GAMESCOPE_COLOR_APP_HDR_METADATA_FEEDBACK", false);
  ctx->atoms.gamescopeColorSliderInUse =
      XInternAtom(ctx->dpy, "GAMESCOPE_COLOR_MANAGEMENT_CHANGING_HINT", false);

  ctx->atoms.gamescopeCreateXWaylandServer =
      XInternAtom(ctx->dpy, "GAMESCOPE_CREATE_XWAYLAND_SERVER", false);
  ctx->atoms.gamescopeCreateXWaylandServerFeedback =
      XInternAtom(ctx->dpy, "GAMESCOPE_CREATE_XWAYLAND_SERVER_FEEDBACK", false);
  ...
  XChangeProperty(ctx->dpy, ctx->root, ctx->atoms.gamescopeXwaylandServerId,
                  XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&serverId,
                  1);
  ...
  if (g_customCursorPath) {
    if (!load_mouse_cursor(ctx->cursor.get(), g_customCursorPath,
                           g_customCursorHotspotX, g_customCursorHotspotY))
...
void update_vrr_atoms(xwayland_ctx_t *root_ctx, bool force,
                      bool *needs_flush = nullptr) {
  bool capable = drm_get_vrr_capable(&g_DRM);
  if (capable != g_bVRRCapable_CachedValue || force) {
    uint32_t capable_value = capable ? 1 : 0;
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeVRRCapable, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&capable_value, 1);
  ...
  if (st2084 != g_bSupportsST2084_CachedValue || force) {
    ...
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeDisplaySupportsHDR, XA_CARDINAL,
                    32, PropModeReplace, (unsigned char *)&hdr_value, 1);
  ...
  if (in_use != g_bVRRInUse_CachedValue || force) {
    ...
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeVRRInUse, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&in_use_value, 1);
  ...
  if (force) {
    ...
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeVRREnabled, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)&enabled_value, 1);
    if (needs_flush)
      *needs_flush = true;
  }
}
...
void update_mode_atoms(xwayland_ctx_t *root_ctx, bool *needs_flush = nullptr) {
  ...
  if (drm_get_screen_type(&g_DRM) == DRM_SCREEN_TYPE_INTERNAL) {
    ...
    uint32_t zero = 0;
    XChangeProperty(root_ctx->dpy, root_ctx->root,
                    root_ctx->atoms.gamescopeDisplayIsExternal, XA_CARDINAL, 32,
  ...
  int len = 0;
  for (int i = 0;
       remaining_size > 0 && i < g_DRM.connector->connector->count_modes; i++) {
    const auto &mode = g_DRM.connector->connector->modes[i];
    int mode_len =
        snprintf(&modes[len], remaining_size, "%s%dx%d@%d", i == 0 ? "" : " ",
                 int(mode.hdisplay), int(mode.vdisplay), int(mode.vrefresh));
    len += mode_len;
    remaining_size -= mode_len;
  }
  XChangeProperty(root_ctx->dpy, root_ctx->root,
                  root_ctx->atoms.gamescopeDisplayModeListExternal, XA_STRING,
                  8, PropModeReplace, (unsigned char *)modes,
                  strlen(modes) + 1);
...
void update_edid_prop() {
  if (!BIsNested()) {
    ...
    for (size_t i = 0; (server = wlserver_get_xwayland_server(i)); i++) {
      ...
      XSetTextProperty(server->ctx->dpy, server->ctx->root, &text_property,
...
void steamcompmgr_main(int argc, char **argv) {
  ...
  while ((o = getopt_long(argc, argv, gamescope_optstring, gamescope_options,
                          &opt_index)) != -1) {
    const char *opt_name;
    switch (o) {
    case 'R':
    ...
    case 0: // long options without a short option
      ...
      if (strcmp(opt_name, "debug-focus") == 0) {
        ...
        sscanf(optarg, "%d,%d", &g_customCursorHotspotX,
  ...
  gamesRunningCount =
      get_prop(root_ctx, root_ctx->root, root_ctx->atoms.gamesRunningAtom, 0);
  overscanScaleRatio = get_prop(root_ctx, root_ctx->root,
                                root_ctx->atoms.screenScaleAtom, 0xFFFFFFFF) /
                       (double)0xFFFFFFFF;
  zoomScaleRatio = get_prop(root_ctx, root_ctx->root,
                            root_ctx->atoms.screenZoomAtom, 0xFFFF) /
                   (double)0xFFFF;
  ...
  for (;;) {
    ...
    for (size_t i = EVENT_X11; i < pollfds.size(); i++) {
      if (pollfds[i].revents & POLLIN) {
        gamescope_xwayland_server_t *server =
            wlserver_get_xwayland_server(i - EVENT_X11);
        assert(server);
        dispatch_x11(server->ctx.get());
      }
    }
    ...
    if (inputCounter != lastPublishedInputCounter) {
      XChangeProperty(root_ctx->dpy, root_ctx->root,
                      root_ctx->atoms.gamescopeInputCounterAtom, XA_CARDINAL,
                      32, PropModeReplace, (unsigned char *)&inputCounter, 1);
    ...
    if (g_bFSRActive != g_bWasFSRActive) {
      ...
      XChangeProperty(root_ctx->dpy, root_ctx->root,
                      root_ctx->atoms.gamescopeFSRFeedback, XA_CARDINAL, 32,
    ...
    g_bOutputHDREnabled =
        (g_bSupportsST2084_CachedValue || g_bForceHDR10OutputDebug) &&
        g_bHDREnabled;

    // Pick our width/height for this potential frame, regardless of how it
    // might change later At some point we might even add proper locking so we
    // get real updates atomically instead of whatever jumble of races the below
    // might cause over a couple of frames
    if (currentOutputWidth != g_nOutputWidth ||
      ...
      if (steamMode && g_nXWaylandCount > 1) {
        ...
        wlserver_set_xwayland_server_mode(0, g_nOutputWidth, g_nOutputHeight,
                                          g_nOutputRefresh);
        wlserver_unlock();
      }
...

      {
        ...
        for (size_t i = 0; (server = wlserver_get_xwayland_server(i)); i++) {
          uint32_t hdr_value =
              (g_bOutputHDREnabled || g_bForceHDRSupportDebug) ? 1 : 0;
          XChangeProperty(server->ctx->dpy, server->ctx->root,
    ...
    // When we observe a new commit being complete for a surface, we ask for a
    // new frame. This ensures that FIFO works properly, since otherwise we
    // might ask for a new frame application can commit a new frame that
    ...
    // Notes:
    //
    // We send the presented event just after the latest latch time possible so
    // PresentWait in Vulkan still returns pretty optimally. The extra 2ms or so
    // can be "display latency" We still provide the predicted TTL refresh time
    // in the presented event though.
    //
    // We ignore or lie most of the flags because they aren't particularly
    // useful for a client to know anyway and it would delay us sending this at
    // an optimal time. (particularly for DXGI frame latency handles under
    // Proton.)
    //
    // The boat is still out as to whether we should do latest latch or
    // pageflip/ttl for the event. For now, going to keep this, and if we change
    // our minds later, it's no big deal.
    //
    // It's a little strange, but we return `presented` for any window not
    // visible and `presented` for anything visible. It's a little disingenuous
    // because we didn't actually show a window if it wasn't visible, but we
    // could! And that is the first opportunity it had. It's confusing but we
    // need this for forward progress.
...

    {
      ...
      if (g_HeldCommits[HELD_COMMIT_BASE]) {
        ...
        if (g_HeldCommits[HELD_COMMIT_BASE]->feedback)
          app_hdr_metadata =
              g_HeldCommits[HELD_COMMIT_BASE]->feedback->hdr_metadata_blob;
      }
      ...
      if (app_hdr_metadata != g_ColorMgmt.pending.appHDRMetadata) {
        if (app_hdr_metadata) {
          ...
          app_hdr_metadata_blob.resize(
              (sizeof(hdr_metadata_infoframe) + (sizeof(uint32_t) - 1)) /
              sizeof(uint32_t));
          memset(app_hdr_metadata_blob.data(), 0,
                 sizeof(uint32_t) * app_hdr_metadata_blob.size());
          memcpy(app_hdr_metadata_blob.data(), &app_hdr_metadata->metadata,
    ...
    // Doesn't realllly matter but avoids an extra frame of being on the wrong
    // window.
    if (focusDirty)
      determine_and_apply_focus();
    ...
    // TODO: Fix properly.
    static bool bHasOverlay =
        (global_focus.overlayWindow && global_focus.overlayWindow->opacity) ||
        (global_focus.externalOverlayWindow &&
         global_focus.externalOverlayWindow->opacity) ||
        (global_focus.overrideWindow && global_focus.focusWindow &&
         !global_focus.focusWindow->isSteamStreamingClient &&
         global_focus.overrideWindow->opacity);

    const bool bSteamOverlayOpen =
        global_focus.overlayWindow && global_focus.overlayWindow->opacity;
    // If we are running behind, allow tearing.
    const bool bSurfaceWantsAsync = (g_HeldCommits[HELD_COMMIT_BASE] &&
                                     g_HeldCommits[HELD_COMMIT_BASE]->async);

    const bool bForceRepaint = g_bForceRepaint.exchange(false);
    const bool bForceSyncFlip =
        bForceRepaint || g_bTakeScreenshot || is_fading_out();
    // If we are compositing, always force sync flips because we currently wait
    // for composition to finish before submitting.
    // If we want to do async + composite, we should set up syncfile stuff and
    // have DRM wait on it.
    const bool bNeedsSyncFlip =
        bForceSyncFlip || g_bCurrentlyCompositing || nIgnoredOverlayRepaints;
    const bool bDoAsyncFlip =
        (((g_nAsyncFlipsEnabled >= 1) && g_bSupportsAsyncFlips &&
          bSurfaceWantsAsync && !bHasOverlay) ||
         bVRR) &&
        !bSteamOverlayOpen && !bNeedsSyncFlip;
    ...
    if (bDoAsyncFlip) {
      ...
      bShouldPaint =
          vblank && (hasRepaint || hasRepaintNonBasePlane || bForceSyncFlip);
    }
...
void steamcompmgr_send_frame_done_to_focus_window() {
  ...
  if (global_focus.focusWindow &&
      global_focus.focusWindow->xwayland().surface.main_surface) {
    wlserver_lock();
    wlserver_send_frame_done(
        global_focus.focusWindow->xwayland().surface.main_surface, &now);
    wlserver_unlock();
  }
}
